...
$(document).ready(function () {
	...
	function calculate() {
		...
		//ALV
		if (parseFloat(localStorage.getItem("i_nehmer_lohn")) < 148200.0) {
			i_alv = 1.1;
		} else { i_alv = 0.5; }
		i_alv_val = localStorage.getItem("i_nehmer_lohn") * 12 * (i_alv / 100.0);


		if (localStorage.getItem("i_nehmer_lohn") * 12 < 24675) {
			i_bvg_obligatorium = 3525;
		} else if (localStorage.getItem("i_nehmer_lohn") * 12 < 84600) {
		...
		//Reallohn
		i_nehmer_lohn_real = i_nehmer_lohn_netto + Number(localStorage.getItem("i_nehmer_spesen"));
	...
	function write() {
		...
		document.getElementById("o_date").innerHTML = localStorage.getItem("i_datum").replace('.', '/');
		...
		now = new Date();
		normDate = ('0' + now.getDate()).slice(-2) + "." + ('0' + (now.getMonth() + 1)).slice(-2) + "." + now.getFullYear();

		document.getElementById("o_ortdate").innerHTML = localStorage.getItem("i_nehmer_ort") + ", " + normDate;
		...
		document.getElementById("o_ahv_perc").innerHTML = parseFloat(i_ahv_iv_eo).toFixed(3);
		...
		document.getElementById("o_alv_perc").innerHTML = parseFloat(i_alv).toFixed(3);
		document.getElementById("o_alv_from").innerHTML = (Number(localStorage.getItem("i_nehmer_lohn")) * 12).toFixed(2);
		document.getElementById("o_alv_val").innerHTML = Number(i_alv_val).toFixed(2);

		document.getElementById("o_bvg_perc").innerHTML = parseFloat(localStorage.getItem("i_nehmer_bvg")).toFixed(3);
		...
		document.getElementById("o_nbu_perc").innerHTML = parseFloat(localStorage.getItem("i_nehmer_nbu")).toFixed(3);
		...
		date_transaction = new Date(localStorage.getItem("i_date_bezahlung"));
		normDate = ('0' + date_transaction.getDate()).slice(-2) + "." + ('0' + (date_transaction.getMonth() + 1)).slice(-2) + "." + date_transaction.getFullYear();

		document.getElementById("o_date_transaction").innerHTML = normDate;
	...
	alert("Um ein PDF zu erhalten oder um die Datei auszudrucken, drücken Sie \"Ctrl\" und \"P\".");
